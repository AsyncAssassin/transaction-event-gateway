...
- `test/app.e2e-spec.ts`: health endpoints (`/health/ready` checks configuration, PostgreSQL and Redis; `/health/serving` leaves Redis out) and recovery of the health pool after PostgreSQL terminates its idle connection; no `X-Powered-By` header and a 65 s keep-alive timeout; `X-Correlation-ID` echoed or generated, including on a 413; malformed JSON and a gzip body that cannot be inflated answered with a generic 400 without parser details; an unknown route answered with 404 `NOT_FOUND`, and an unsupported `Content-Encoding` or charset with 415 `UNSUPPORTED_MEDIA_TYPE`; a 200 KB body accepted by the parser.
- `test/payment-intents.e2e-spec.ts`: creation with the full response (canonical amount, `confirmedTxHash` null, `updatedAt` equal to `createdAt`); canonical amounts in creation and read responses; a read by ID in the same form as the creation response, a read after the worker's columns are set to `CONFIRMED`, an unknown ID (404 `NOT_FOUND`) and an ID that is not a UUID (400); a missing `Idempotency-Key`; DTO errors with `details`; an oversized body (413); replay with `Idempotent-Replayed: true`; a conflict that creates no second intent; `"125.50"` and `"125.5"` treated as different bodies; concurrent duplicates (one 201, one 200, one intent) and concurrent requests with one key and different bodies (201 and 409).
- `test/webhooks.e2e-spec.ts`: acceptance writes one `RECEIVED` inbox row and one `PENDING` outbox row with `{ webhookEventId, correlationId }`, using the request's correlation ID or the generated one, and `webhook_accepted` and `webhook_replayed` log the webhook event ID; an invalid signature (401), a stale timestamp (400), an invalid DTO, an unknown `reference` field and a transaction hash with whitespace inside (400) write nothing; the transaction hash is stored in canonical form, and a redelivery that spells it differently is `ALREADY_ACCEPTED`, as is a redelivery of an event stored before hashes were normalized; a duplicate gets `ALREADY_ACCEPTED`; an event ID conflict and a nonce replay get 409; concurrent duplicate deliveries, and concurrent events that share a nonce, leave one inbox row and one outbox row.
- `test/outbox.e2e-spec.ts`: `OutboxDispatcherService` against PostgreSQL and Redis. Publishing creates one job with the payload's `webhookEventId` and `correlationId` (and no correlation ID when the payload has none or one that is not a safe header value), logs `outbox_dispatch_published` under that correlation ID, uses 5 attempts and exponential backoff from 5 s, and marks the outbox row `PUBLISHED` and the webhook `QUEUED`. A publish failure records `FAILED`, `attempts`, a sanitized `last_error` and a later `next_attempt_at`, and leaves the webhook `RECEIVED`; transient failures stay retryable at 10 attempts; a failed publish ends the batch and leaves the remaining rows for the next run; a poison payload gets `dead_at` and is not selected again; a due retry publishes. Reconciliation hands stale `PUBLISHED` rows back to the dispatcher only while their webhook is `RECEIVED` or `QUEUED`, leaves recent, finished and unpublished rows alone, respects the batch size, never re-queues a row twice across concurrent runs, and logs the correlation ID of each row it hands back.
...
- `test/worker-bullmq.e2e-spec.ts`: the BullMQ consumer in `WorkerModule` processes a job added to the queue and confirms the payment intent, for a job without a correlation ID and for one whose `worker_job_processed` line carries it.
- `test/worker-processing.e2e-spec.ts`: `WebhookEventProcessorService` against PostgreSQL, without a queue. A confirmation with a `SUCCEEDED` attempt; a second run returns `already_processed` without side effects; all seven failure reasons (`PAYMENT_INTENT_TERMINAL` through `FAILED` and `EXPIRED` intents); a `PROCESSING` intent is confirmed; a later job for a `FAILED` event records a `FAILED` attempt with the stored reason and changes nothing, even after the payment intent it named was created; another spelling of a hash that confirmed another intent is `CONFIRMED_TX_HASH_CONFLICT`, and an intent accepts its own hash in another spelling, also when its stored hash predates normalization; an exception at the first write, or at the final attempt insert after the intent was updated, rolls everything back, including the attempt row.
- `test/openapi.e2e-spec.ts`: the document served at `/docs/openapi.json` declares the webhook request body, a schema for every success response, the error envelope for each documented error status with `Retry-After` on 429, and the payment intent ID as a UUID path parameter; the payment intent schema has exactly the fields of a real creation response, with the nullable ones marked.
- `test/tx-hash-migration.e2e-spec.ts`: the SQL of the `NormalizeConfirmedTxHashes` migration computes the same canonical hash as `normalizeTxHash` for every character that JavaScript trims and for three it keeps; the migration rewrites stored hashes in canonical form (uppercase, padded and Unicode-padded hexadecimal, a padded base58 value keeping its case), gives a hash that several intents share to only one of them (the one that already has it, or else the earliest), and changes nothing when it runs again.
...
- `test/rate-limit.e2e-spec.ts`: with rate limiting enabled and a limit of 3, the fourth and fifth requests get 429 `RATE_LIMITED` with a `Retry-After` header, and health endpoints are not limited. Only the in-memory limiter keyed by the observed client IP is covered.
...
- Transaction hashes (`src/webhooks/tx-hash.spec.ts`): trimming, lowercase `0x`-prefixed hexadecimal, and unchanged case for other formats.
...
- Request context and exception filter: accepted, generated and replaced correlation IDs; background work under a stored correlation ID with a new request ID, and without a context for a missing or unsafe one; a generic 400 for parser errors; 503 for PostgreSQL connection failures; the envelope for Nest's default 404 and 5xx bodies and for the rate limiter's 429; body parser and zlib errors mapped by status (400, 413, 415) without their messages; a generic 500 that logs only stable codes.
...
3. creates a payment intent and checks the canonical amount in the response, then its replay (200 with `Idempotent-Replayed: true`) and a conflicting request (409);
...
6. reads the payment intent with `GET /payment-intents/{id}` and checks `CONFIRMED` and the webhook's transaction hash.
