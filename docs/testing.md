...
- `test/app.e2e-spec.ts`: health endpoints (`/health/ready` checks configuration, PostgreSQL and Redis; `/health/serving` leaves Redis out) and recovery of the health pool after PostgreSQL terminates its idle connection; `X-Correlation-ID` echoed or generated, including on a 413; malformed JSON answered with a generic 400 without parser details; a 200 KB body accepted by the parser.
