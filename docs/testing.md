...
- `test/tx-hash-migration.e2e-spec.ts`: the `NormalizeConfirmedTxHashes` migration rewrites stored hashes in canonical form (uppercase and padded hexadecimal, a padded base58 value keeping its case) and leaves hashes that two intents share unchanged.
