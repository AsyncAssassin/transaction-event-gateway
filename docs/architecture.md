...
- **Trust in the provider.** A signed `transaction.confirmed` event is accepted as proof of payment. The worker checks its amount and asset against the payment intent, but it does not check `destination`, because the signed webhook contract does not carry one, and it does not look the transaction up on a blockchain. Whoever holds `WEBHOOK_SECRET` can therefore confirm any payment intent.
