...
sequenceDiagram
    ...
    alt a row was inserted
        A->>DB: INSERT outbox_events as PENDING with payload webhookEventId
        A->>DB: COMMIT
        A-->>P: 202 ACCEPTED
...
- **Jobs carry IDs only.** Job data and the outbox payload are `{ webhookEventId }`. The worker reloads the event and the payment intent from PostgreSQL under row locks, so a stale or duplicated job cannot apply outdated data.
...
- **Error responses.** Unexpected errors return a generic 500 body and malformed JSON a generic 400; stack traces, parser details and database messages are not returned.
...
- **Log format.** Each application event is a single-line JSON object built by `src/common/logging/structured-logger.ts`, with an `event` name and allow-listed fields. It is passed as the message to Nest's default `ConsoleLogger`, which prefixes it with `[Nest]`, the process ID, a local timestamp, the log level and the logger context, and adds ANSI colors unless `NO_COLOR` is set. A log line is therefore not a pure JSON line. Startup messages from Nest and from `main.ts` and `worker.ts` are plain text.
...
- **Correlation IDs.** The API accepts `X-Correlation-ID` (1 to 255 visible ASCII characters) or generates a UUID, returns it in the response header and in error bodies, and keeps it in `AsyncLocalStorage` for the duration of the request (`src/common/request-context/`). Log lines written while the request is handled, such as `payment_intent_created` or `webhook_accepted`, carry `correlationId` and a per-request `requestId`. The ID is not propagated into outbox payloads or BullMQ jobs, which carry only `{ webhookEventId }`.
- **Following a webhook.** The API's `webhook_accepted` line carries `provider`, `externalEventId` and `status`, but not `webhookEventId`. Dispatcher lines carry `webhookEventId`; the worker's job lines carry `webhookEventId` and `jobId`. Join them through the `webhook_events` row, from `external_event_id` to `id`.
